...
$(function() {
    ...
    $('#btn-unterschrift-kundendienst-speichern').click(function() {
        ...
        $.ajax({
            type: 'POST',
            url: '/services/unterschrift_speichern.php',
            data: {
            ...
            success: function(msg) {
                ...
                $('#img-unterschrift-kundendienst').attr('src', '/upload/unterschrift_kundendienst_' + $('#aid').val() + '.png?' + (new Date()).getTime()).removeClass('hidden');
            },
    ...
    $('#btn-unterschrift-kunde-speichern').click(function() {
        ...
        $.ajax({
            ...
            success: function(msg) {
                ...
                $('#img-unterschrift-kunde').attr('src', '/upload/unterschrift_kunde_' + $('#aid').val() + '.png?' + (new Date()).getTime()).removeClass('hidden');
            },
            error: function(obj, text, error) {
    ...
    $('#arbeitsbericht-form #speichern').click(function() {
        arbeitsberichtSpeichern();
    });
    $('#arbeitsbericht-form #speichern-und-senden').click(function() {
...
function stundenRechnen(reiseVon, arbeitVon, arbeitBis, reiseBis, reisestunden, arbeitsstunden, ueberstunden) {
    ...
    var reiseStundenSumme = moment.duration();
    $('.zeit-reisestunden').each(function() {
        ...
        reiseStundenSumme += moment.duration(moment($(this).val(), 'HH:mm'));
    });
    $('#summe-reisestunden').val(moment.utc(reiseStundenSumme).format('HH:mm'));
    
    // Arbeitsstunden aufaddieren
    var arbeitsStundenSumme = moment.duration();
    $('.zeit-arbeitsstunden').each(function() {
        
        // Die Vorbereitungszeile nicht mit in die Summe einbeziehen
        if($(this).parents('tr.arbeitszeit-line').hasClass('prepare')) return;
        
        arbeitsStundenSumme += moment.duration(moment($(this).val(), 'HH:mm'));
    });
...
function addArbeitszeiten() {
    ...
    code += '<input type="text" class="form-control" name="arbeitszeit-datum[]" value="' + $('#arbeitszeit-datum-prepare').val() + '"/>';
    code += '</td>';
    code += '<td>';
...
function arbeitsberichtLoeschen() {
    if(confirm('Sind Sie sicher, daß Sie diesen Bericht löschen möchten?')) {
        $.ajax({
            type: 'POST',
            url: '/services/arbeitsbericht_loeschen.php',
            data: {
                'arbeitsbericht_id': $('#aid').val()
            },
            success: function() {
...
function unAssignFile(id) {
    $.ajax({
        type: 'POST',
        url: '/services/arbeitsbericht_datei_entfernen.php',
        data: {
            'berichtId': $('#aid').val(),
            'documentId': id
        },
